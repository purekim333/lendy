...
export default function AdminProductCreate() {
  ...
  const onSubmit = async () => {
    ...
    // IMPORTANT: Backend currently supports single color/tag per product entity.
    // Mapping first available color and tag.
    const primaryColor = colors[0] || "Black";
    const primaryTag = tags[0] || "Daily";

    // Filter variants to match primary color (or send all if backend logic changes)
    // For now, sending all, assuming backend creates options for them.
    // However, backend DTO for option is: size, count, buyPrice, rentalPrice.
    // Mapping variants to options.

    const optionsPayload = variants.map(v => ({
      size: v.size,
      count: v.stock,
      buyPrice: price,   // Using product base price
      rentalPrice: rentalPrice || 0 // Using product base rental price
    }));

    const payload = {
      name: title,
      type: category, // Mapping category to type
      tag: primaryTag,
      color: primaryColor,
      buyPrice: Number(price),
      rentalPrice: Number(rentalPrice) || 0,
      description,
      // Default to 1 (Normal) if not selected
    ...
    try {
      const res = await fetch("/api/v1/admin/products/create", { method: "POST", body: form });
    ...
    <div className="mx-auto max-w-6xl p-4 sm:p-6 space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <Section title="기본 정보">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="flex flex-col">
                <label className="label">카테고리 (Type)</label>
                <select className="input" value={category} onChange={e => setCategory(e.target.value)}>
                  {["Tops", "Bottoms", "Outers"].map(c => <option key={c} value={c}>{c}</option>)}
                </select>
              </div>

              <TextField label="태그 (첫번째 태그 사용)" value={tagsInput} onChange={setTagsInput} placeholder="예) Daily, Travel" />
            </div>
          </Section>
          ...
          <Section title="가격">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <NumberField label="판매가 (Buy Price)" value={price} onChange={v => setPrice(v === "" ? "" : toNum(v))} min={0} required />
              <NumberField label="대여가 (Rental Price)" value={rentalPrice} onChange={v => setRentalPrice(v === "" ? "" : toNum(v))} min={0} />
            </div>
          </Section>

          <Section title="옵션 → 변형(Variants)">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <TextField label="색상 목록 (첫번째 색상 사용)" value={colorsInput} onChange={setColorsInput} placeholder="예) Black" />
          ...
          <Section title="제품 상세정보 (1: 매우 큼/두꺼움/높음 ~ 5: 매우 작음/얇음/낮음)">
            {/* Mapping string literals to byte logic in Product entity, assuming 1-5 scale or similar logic is desired? 
                 Product entity uses Byte. Let's assume 1-3 or 1-5. 
                 Code uses: 
                 thickness, elasticity, lining, handFeel, seeThrough.
                 Let's provide simple dropdowns 1-3.
              */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <SelectField label="두께 (1:두꺼움 ~ 3:얇음)" value={(details.thickness || "").toString()} onChange={v => setDetails(d => ({ ...d, thickness: toNum(v) }))} options={[["1", "1 (두꺼움)"], ["2", "2 (보통)"], ["3", "3 (얇음)"]]} />
              <SelectField label="신축성" value={(details.elasticity || "").toString()} onChange={v => setDetails(d => ({ ...d, elasticity: toNum(v) }))} options={[["1", "1 (좋음)"], ["2", "2 (보통)"], ["3", "3 (없음)"]]} />
              <SelectField label="안감" value={(details.lining || "").toString()} onChange={v => setDetails(d => ({ ...d, lining: toNum(v) }))} options={[["1", "1 (있음)"], ["2", "2 (부분)"], ["3", "3 (없음)"]]} />
              <SelectField label="촉감" value={(details.handFeel || "").toString()} onChange={v => setDetails(d => ({ ...d, handFeel: toNum(v) }))} options={[["1", "1 (부드러움)"], ["2", "2 (보통)"], ["3", "3 (거지)"]]} />
              <SelectField label="비침" value={(details.seeThrough || "").toString()} onChange={v => setDetails(d => ({ ...d, seeThrough: toNum(v) }))} options={[["1", "1 (없음)"], ["2", "2 (약간)"], ["3", "3 (있음)"]]} />
            </div>
          </Section>
        </div>
